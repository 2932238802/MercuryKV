...
{
  public:
    StreamEchoReader(ResponseStreamPtr respStream) : respStream_(std::move(respStream))
    {
    }
...
class RequestStreamExampleCtrl : public HttpController<RequestStreamExampleCtrl>
{
  public:
    ...
    ADD_METHOD_TO(RequestStreamExampleCtrl::stream_upload, "/stream_upload", Post);
    METHOD_LIST_END

    void stream_echo(const HttpRequestPtr &, RequestStreamPtr &&stream,
                     std::function<void(const HttpResponsePtr &)> &&callback) const
    {
        auto resp = drogon::HttpResponse::newAsyncStreamResponse(
            [stream](ResponseStreamPtr respStream) {
                stream->setStreamReader(std::make_shared<StreamEchoReader>(std::move(respStream)));
            });
        callback(resp);
    }

    void stream_upload(const HttpRequestPtr &req, RequestStreamPtr &&stream,
                       std::function<void(const HttpResponsePtr &)> &&callback) const
    {
        struct Entry
        {
        ...
        auto reader = RequestStreamReader::newMultipartReader(
            ...
            {
                LOG_INFO << "Multipart name: " << header.name << ", filename:" << header.filename
                ...
                if (!files->back().header.filename.empty())
                {
                    ...
                    files->back().file.open("uploads/" + tmpName, std::ios::trunc);
                }
            },
            ...
            [files, callback = std::move(callback)](std::exception_ptr ex)
            {
                ...
                {
                    LOG_INFO << "stream finish, received " << files->size() << " files";
                    Json::Value respJson;
